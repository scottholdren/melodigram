...
header.innerHTML = `<h1>Piano Workshop</h1><div style="display:flex;gap:1rem"><a href="workshop.html" style="color:#5566ff;text-decoration:none;font-size:0.75rem">Multi-instrument</a><a href="index.html" style="color:#5566ff;text-decoration:none;font-size:0.75rem">Game</a></div>`;
...
document.getElementById("btn-export")!.addEventListener("click", () => {
  ...
  // Emit rows as pianoRow() calls — all rows in this workshop are piano pitches
  const rowsArr = pitches
    .map((p) => `    pianoRow("${p}")`)
    .join(",\n");
